...
public sealed class ClaudeBrain
{
    ...
    const string SystemPreamble = """
        ...
        - open_app: para lanzar un PROGRAMA de escritorio suelto. target = ejecutable o nombre de app de Windows (firefox, chrome, msedge, code, notepad, calc, explorer, spotify, ...). Deduce el nombre del ejecutable de lo que diga el usuario. Si el usuario nombra una WEB o servicio conocido que se usa desde el navegador (YouTube, Gmail, Twitter/X, Instagram, Facebook, Netflix, WhatsApp Web, Amazon, GitHub, ...) — "abre youtube", "entra a gmail" — NUNCA pongas solo el navegador (msedge/chrome/firefox) sin más: pon como target la URL completa de esa web (p. ej. "https://youtube.com", "https://mail.google.com"), así se abre directo en esa página, no en blanco ni en una búsqueda.
        - web_search: para ABRIR una búsqueda en el navegador cuando el usuario quiere BUSCAR ALGO (términos, no el nombre de una web conocida) y navegar él mismo ("busca vuelos a Cusco", "ábreme una búsqueda de..."). target = términos de búsqueda. No la uses para abrir una web con nombre propio: eso es open_app con la URL.
